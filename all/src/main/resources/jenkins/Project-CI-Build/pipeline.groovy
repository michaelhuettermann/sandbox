    ...
    stage('SonarQube analysis') {
        withCredentials([usernamePassword(credentialsId: 'SONAR', passwordVariable: 'SONAR_PW', usernameVariable: 'SONAR_UN')]) {
            withSonarQubeEnv('Sonar') {
                sh 'mvn sonar:sonar -Dsonar.host.url=http://localhost:9000 -Dsonar.login=$SONAR_UN -Dsonar.password=$SONAR_PW -f all/pom.xml -Dsonar.projectKey=com.huettermann:all:master -Dsonar.language=java -Dsonar.junit.reportPaths=target/surefire-reports -Dsonar.jacoco.reportPaths=target/jacoco.exec'
            }
